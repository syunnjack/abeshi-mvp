...
const productFiles = await listJsonFiles(path.join(root, 'content', 'products'));
const productPayloads = await Promise.all(productFiles.map(async file => JSON.parse(await fs.readFile(file, 'utf8'))));
const productRecords = productPayloads.flatMap(payload => (
  Array.isArray(payload) ? payload : Array.isArray(payload.catalogs) ? payload.catalogs : [payload]
));
const productsByKey = new Map(productRecords.map(record => [`${record.site}:${record.slug}`, record]));
...
function decisionAssistant(site, topic, slug) {
  const catalog = productsByKey.get(`${site.slug}:${slug}`);
  const candidates = catalog?.candidates?.length
    ? `<div class="candidate-match"><div class="candidate-heading"><div><p class="result-kicker">MATCHED PRODUCTS & SERVICES</p><h4>条件に合いやすい具体候補</h4></div><p>診断結果に近い候補から順に並びます。価格・在庫・契約条件は公式ページで再確認してください。</p></div><div class="candidate-list" data-candidate-list>${catalog.candidates.map(candidate => `<article class="candidate-card" data-candidate data-fit="${escapeHtml(candidate.fit)}"><div><span data-match-label>比較候補</span><small>${escapeHtml(candidate.type)}</small></div><h5>${escapeHtml(candidate.name)}</h5><p class="candidate-provider">${escapeHtml(candidate.provider)} · ${escapeHtml(candidate.price)}</p><p>${escapeHtml(candidate.reason)}</p><aside>${escapeHtml(candidate.caution)}</aside><a href="${escapeHtml(candidate.url)}" target="_blank" rel="noopener" data-product-link>公式ページで確認 →</a></article>`).join('')}</div><small class="catalog-date">候補情報の確認日: ${escapeHtml(catalog.checkedAt)}／広告報酬ではなく入力条件との適合で並べ替えています。</small></div>`
    : '<div class="candidate-empty"><b>具体候補を確認中です</b><p>このテーマの商品・サービスは、公式情報を確認できたものから追加します。</p></div>';
  return `<section class="decision-tool" data-risk="${escapeHtml(site.risk)}" data-topic="${escapeHtml(topic)}"><div class="decision-intro"><p class="eyebrow">PERSONAL CHOICE SIMULATOR</p><h2>${escapeHtml(topic)}のかんたん診断</h2><p>4つの条件から3つの選び方を自動採点し、あなたの条件で合いやすい案と具体候補を表示します。</p></div><form class="decision-form"><label><span>いちばん重視すること</span><select name="priority"><option value="balance">費用・使いやすさ・安心のバランス</option><option value="cost">料金を抑えること</option><option value="quality">品質・安心・サポート</option></select></label><label><span>1か月の利用回数</span><select name="frequency"><option value="1">1回程度</option><option value="4" selected>4回程度</option><option value="8">8回程度</option><option value="20">20回以上</option></select></label><label><span>月額・維持費の上限（円）</span><input name="monthlyBudget" type="number" min="0" max="100000000" step="100" inputmode="numeric" placeholder="例：10000"></label><label><span>変更・解約のしやすさ</span><select name="flexibility"><option value="high">とても重視する</option><option value="normal" selected>ある程度重視する</option><option value="low">長期利用を前提にする</option></select></label></form><p class="decision-privacy">入力内容はこの端末内だけで計算され、送信・保存されません。</p><aside class="decision-result" aria-live="polite"><p class="result-kicker">入力条件での暫定最適解</p><h3 data-decision-title>計算しています</h3><p data-decision-summary></p><div class="simulation-grid"><div><small>適合度</small><strong data-decision-score>—</strong></div><div><small>月の検討上限</small><strong data-decision-monthly>未入力</strong></div><div><small>年間予算枠</small><strong data-decision-annual>未入力</strong></div><div><small>1回あたり目安</small><strong data-decision-per-use>未入力</strong></div></div><div class="score-list" data-decision-ranking></div><h4>この結果になった理由</h4><ul data-decision-reasons></ul><h4>次に確認すること</h4><ul data-decision-checks></ul><p class="decision-warning" data-decision-warning></p></aside>${candidates}</section>`;
}
...
document.querySelectorAll('.decision-tool').forEach(tool => {
  ...
  const update = () => {
    ...
    const candidates = [...tool.querySelectorAll('[data-candidate]')];
    if (candidates.length) {
      const routeOrder = result.ranked.map(option => option.id);
      candidates.sort((left, right) => routeOrder.indexOf(left.dataset.fit) - routeOrder.indexOf(right.dataset.fit));
      const candidateList = tool.querySelector('[data-candidate-list]');
      candidates.forEach((candidate, index) => {
        candidate.classList.toggle('is-match', index === 0);
        candidate.querySelector('[data-match-label]').textContent = index === 0 ? '今回の第一候補' : '比較候補 ' + (index + 1);
...
const candidateCss = `.candidate-match{margin-top:24px;border-top:1px solid var(--line);padding-top:28px}.candidate-heading{display:grid;grid-template-columns:1fr 1fr;gap:25px;align-items:end}.candidate-heading h4{font-size:22px;margin:7px 0}.candidate-heading>p{font-size:10px;line-height:1.8;color:var(--muted)}.candidate-list{display:grid;grid-template-columns:repeat(3,1fr);gap:10px;margin-top:18px}.candidate-card{display:flex;flex-direction:column;background:var(--paper);border:1px solid var(--line);padding:19px;min-height:330px;transition:transform .2s,border-color .2s}.candidate-card.is-match{border:2px solid var(--accent);transform:translateY(-4px);background:#fff}.candidate-card>div{display:flex;justify-content:space-between;gap:8px}.candidate-card>div span{background:#68736f;color:#fff;padding:5px 7px;font-size:8px}.candidate-card.is-match>div span{background:var(--accent)}.candidate-card>div small{font-size:8px;color:var(--muted)}.candidate-card h5{font-size:18px;margin:15px 0 5px}.candidate-card p{font-size:10px;line-height:1.7}.candidate-provider{color:var(--accent);font-weight:700}.candidate-card aside{font-size:9px;line-height:1.7;color:var(--muted);border-top:1px solid var(--line);padding-top:12px}.candidate-card a{margin-top:auto;background:var(--ink);color:#fff;text-decoration:none;padding:12px;text-align:center;font-size:9px;font-weight:800}.catalog-date{display:block;color:var(--muted);font-size:8px;margin-top:12px}.candidate-empty{margin-top:24px;background:var(--surface);padding:18px}.candidate-empty b{font-size:11px}.candidate-empty p{font-size:9px;color:var(--muted)}@media(max-width:780px){.candidate-heading,.candidate-list{grid-template-columns:1fr}.candidate-card{min-height:auto}.candidate-card.is-match{transform:none}}`;
...
for (const site of portfolio.verticals) {
  ...
  await write(siteOutput, 'assets/site.css', css + decisionCss + candidateCss);
